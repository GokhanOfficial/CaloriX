...
CREATE TABLE public.profiles (
  ...
  -- Calculated targets (can be AI-generated or manual)
  bmr NUMERIC(6,1),
  tdee NUMERIC(6,1),
...
-- =============================================
-- FOOD NUTRITION TABLE (per 100g/100ml)
-- =============================================
...
CREATE POLICY "Users can update foods they created"
  ON public.foods FOR UPDATE
  TO authenticated
  USING (created_by = auth.uid());

-- Food nutrition policies
CREATE POLICY "Anyone can view food nutrition"
  ON public.food_nutrition FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Authenticated users can add food nutrition"
...
CREATE POLICY "Users can update their own recent foods"
  ON public.recent_foods FOR UPDATE
  USING (auth.uid() = user_id);
...
-- Function to increment food popularity
...
-- Trigger to increment food popularity when meal entry is added
CREATE TRIGGER on_meal_entry_created
  AFTER INSERT ON public.meal_entries
  FOR EACH ROW EXECUTE FUNCTION public.increment_food_popularity();